	     ...
	     unsigned vsel)
{
	...
	ret = regmap_write(rdev->regmap, MAX77720_REG_CNFG_DCDC1, ((vsel & MAX77720_BIT_VOUT_IBB) >> 8));
	if (ret < 0) {
		return ret;
	}
...
static int max77720_regulator_probe(struct i2c_client *client)
{
	...
	if (config.ena_gpiod) {
		if (!pdata->enable_external_control) {
			gpiod_set_value_cansleep(config.ena_gpiod, 0);            //no need external gpio control so put it low
	...
	ret = regmap_write(config.regmap, MAX77720_REG_CNFG_GLBL, pdata->cnfg_glbl_flags);  //by EN or force enable
	if (ret < 0) {
